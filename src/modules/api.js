...
module.exports = (config,client) => {
  ...
  app.post('/api/v1/verifyUser', async (req, res) => {
    try {
      ...
      if (req.query.guild_id === undefined) return res.status(400).send({
        success: 'false',
        message: "The argument 'guild_id' is required",
      })
      if (req.query.user_id === undefined) return res.status(400).send({
        success: 'false',
        message: "The argument 'user_id' is required",
      })

      const guild = client.guilds.cache.get(req.query.guild_id);
      if (guild === undefined) return res.status(404).send({
        success: 'false',
      ...
      if (member === undefined) return res.status(404).send({
        success: 'false',
